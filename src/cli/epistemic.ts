...
function resolveDefaultModel(): string | undefined {
  ...
  const has = (envKey: string | null, provider: string) =>
    (!!envKey && !!process.env[envKey] && process.env[envKey]!.trim() !== "") || provider in authed;

  if (has(null, "openai-codex")) return DEFAULT_MODEL;
  if (has("OPENROUTER_API_KEY", "openrouter")) return "openrouter/deepseek/deepseek-v4-pro";
